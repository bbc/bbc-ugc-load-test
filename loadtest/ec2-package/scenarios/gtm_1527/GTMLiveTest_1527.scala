...
//class GTMLiveLive(val run_time: Int = 60) extends Simulation {
class GTMLiveTest_1527 extends Simulation {
  ...
  val scaleDown = 4 // Use 1 for no scale down, otherwise to run lower level load tests
  val run_time = 10
  val rampUpTime = 5
  val reuseIpPct: Float = 0.2.toFloat

  val newsRps = 1000 / scaleDown
  //  val newsRps = 300 / scaleDown // 30% - for altered status mix
  val sportRps = 50 / scaleDown // 4.5%
  val homePageRps = 50 / scaleDown // 4.5%
  val fourFourRps = 15 / scaleDown // 0.5%
  //  val fourFourRps = 5 / scaleDown // 10% - for altered status mix
  val threeOneRps = 15 / scaleDown // 0.5%
  //  val threeOneRps = 600 / scaleDown // 60% - for altered status mix
  ...
  val four_o_four_expected_requests_per_second = fourFourRps / num_instances

  val three_o_one_expected_requests_per_second = threeOneRps / num_instances

  val euWestHttpConf = http
    .baseURL("https://www.bbc.com.europe-west-1.live-test.gtm-edge.e9fb45cfcc47b85b.xhst.bbci.co.uk")
    ...
    .header("Host", "www.stage.bbc.com")

  val euCentralHttpConf = http
    .baseURL("https://www.bbc.com.europe-central-1.live-test.gtm-edge.e9fb45cfcc47b85b.xhst.bbci.co.uk")
    .acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")
    .doNotTrackHeader("1")
    .acceptLanguageHeader("en-GB,en-US;q=0.8,en;q=0.6")
    .acceptEncodingHeader("gzip, deflate")
  ...
  val scn_three_o_one = scenario("threeoone")
    .exec(rand_octets(reuseIpPct))
    .exec(http("threeoone")
      .get("/news/")
      .check(status.in(301, 200))
      .header("my_ip", "${octet_1}.${octet_2}.${octet_3}.${octet_4}"))
  ...
  setUp(
    ...
    scn_three_o_one.inject(
      rampUsersPerSec(1) to(three_o_one_expected_requests_per_second) during(rampUpTime minutes),
      constantUsersPerSec(three_o_one_expected_requests_per_second) during(run_time minutes)
    )
  ).protocols(
    euWestHttpConf
