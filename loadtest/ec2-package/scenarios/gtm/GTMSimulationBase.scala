...
class GTMSimulationBase(val scale_down_factor: Int = 10, val scale_up_factor: Int = 0, val run_time: Int, val scheme: String = "https") extends Simulation {

  val num_instances = Integer.getInteger("numEC2Instances")

  val news_expected_requests_per_second = 249 / num_instances
  ...
  val sport_expected_requests_per_second = 59 / num_instances
  ...
  val iplayer_expected_requests_per_second = 56 / num_instances
  ...
  val weather_expected_requests_per_second = 35 / num_instances
  ...
  val radio_expected_requests_per_second = 12 / num_instances
  ...
  val root_expected_requests_per_second = 10 / num_instances
  ...
  val httpConf = http
    .baseURL(scheme + "://www.bbc.europe-west-1.live-int.gtm-edge.e9fb45cfcc47b85b.xhst.bbci.co.uk")
  ...
  if (scale_up_factor == 0) {
    setUp(
        scn_news.inject(
            rampUsersPerSec(1) to(news_expected_requests_per_second_scaled_down) during(rampUpTime minutes),
            constantUsersPerSec(news_expected_requests_per_second_scaled_down) during(run_time minutes)
        ),
        scn_sport.inject(
            rampUsersPerSec(1) to(sport_expected_requests_per_second_scaled_down) during(rampUpTime minutes),
            constantUsersPerSec(sport_expected_requests_per_second_scaled_down) during(run_time minutes)
        ),
        scn_iplayer.inject(
            rampUsersPerSec(1) to(iplayer_expected_requests_per_second_scaled_down) during(rampUpTime minutes),
            constantUsersPerSec(iplayer_expected_requests_per_second_scaled_down) during(run_time minutes)
        ),
        scn_weather.inject(
            rampUsersPerSec(1) to(weather_expected_requests_per_second_scaled_down) during(rampUpTime minutes),
            constantUsersPerSec(weather_expected_requests_per_second_scaled_down) during(run_time minutes)
        ),
        scn_radio.inject(
            rampUsersPerSec(1) to(radio_expected_requests_per_second_scaled_down) during(rampUpTime minutes),
            constantUsersPerSec(radio_expected_requests_per_second_scaled_down) during(run_time minutes)
        ),
        scn_root.inject(
...
//     )
    ...
    setUp(
        scn_news.inject(
            rampUsersPerSec(1) to(news_expected_requests_per_second_scaled_up) during(rampUpTime minutes),
            constantUsersPerSec(news_expected_requests_per_second_scaled_up) during(run_time minutes)
        ),
        scn_sport.inject(
            rampUsersPerSec(1) to(sport_expected_requests_per_second_scaled_up) during(rampUpTime minutes),
            constantUsersPerSec(sport_expected_requests_per_second_scaled_up) during(run_time minutes)
        ),
        scn_iplayer.inject(
            rampUsersPerSec(1) to(iplayer_expected_requests_per_second_scaled_up) during(rampUpTime minutes),
            constantUsersPerSec(iplayer_expected_requests_per_second_scaled_up) during(run_time minutes)
        ),
        scn_weather.inject(
            rampUsersPerSec(1) to(weather_expected_requests_per_second_scaled_up) during(rampUpTime minutes),
            constantUsersPerSec(weather_expected_requests_per_second_scaled_up) during(run_time minutes)
        ),
        scn_radio.inject(
            rampUsersPerSec(1) to(radio_expected_requests_per_second_scaled_up) during(rampUpTime minutes),
            constantUsersPerSec(radio_expected_requests_per_second_scaled_up) during(run_time minutes)
        ),
        scn_root.inject(
